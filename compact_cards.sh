#!/bin/bash
echo "🔧 Делаю компактные карточки..."

# Резервная копия
cp app.js app.js.backup.compact.$(date +%s)
cp style.css style.css.backup.compact.$(date +%s)
echo "💾 Бэкап создан"

# 1. Обновляем стили для компактных карточек
cat >> style.css << 'CSS'
...
CSS
echo "✅ Стили для компактных карточек добавлены"

# 2. Обновляем app.js - меняем рендер записей
cat > app.js << 'JS'
// === ГДЕСВЕТА v2.1 — КОМПАКТНЫЕ КАРТОЧКИ ===
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('✅ ГдеСвета v2.1 запущена');
  migrateData();
  setupTabs();
...
JS
echo "✅ app.js обновлён с компактными карточками"
...
echo ""
echo "✅ КОМПАКТНЫЕ КАРТОЧКИ ГОТОВЫ!"
echo "✨ По умолчанию: только имя и время"
echo "✨ Нажми на карточку → раскроется полная информация"
