#!/bin/bash
echo "🚀 Обновление до версии 2.0..."

# 1. Резервная копия
cp app.js app.js.backup.v1.$(date +%s)
cp style.css style.css.backup.v1.$(date +%s)
echo "💾 Бэкап создан"

# 2. Добавляем стили для статусов и длительности
cat >> style.css << 'CSS'
...
.free-slot-btn:hover { background: #4caf50; color: white; }
...
# 3. Полностью переписываем app.js с новыми функциями
cat > app.js << 'JS'
// === ГДЕСВЕТА v2.0 — С ДЛИТЕЛЬНОСТЬЮ, СТАТУСАМИ И КОНТРОЛЕМ ВРЕМЕНИ ===
const STORAGE_KEY = 'gdesveta_data';
const WORK_START = 9;  // Рабочий день с 9:00
const WORK_END = 21;   // до 21:00
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('✅ ГдеСвета v2.0 запущена');
  migrateData();
  setupTabs();
...
function renderDayEntries() {
  ...
  container.innerHTML = `<h3 style="margin:15px 0 10px;font-size:16px;">${formatDate(state.selectedDate)}</h3>` + 
    entries.map(e => {
      ...
      return `
      <div class="entry-card status-${e.status}">
        <div class="entry-header">
          <span class="entry-time">${e.time}${endTime ? ' - ' + endTime : ''}</span>
          <span>${e.price}₽</span>
        </div>
        <div class="entry-name">
          ${e.name}
          <span class="status-badge status-${e.status}">${statusLabels[e.status]}</span>
        </div>
        <div class="entry-details">
          ${e.service}${e.zone ? ' · ' + e.zone : ''}${e.phone ? ' · 📞 ' + e.phone : ''}
          · ⏱️ ${e.duration} мин
        </div>
        ${e.notes ? `<div class="entry-details" style="margin-top:4px;font-style:italic;">💬 ${e.notes}</div>` : ''}
        
        <div class="status-buttons">
          <button class="status-btn ${e.status==='new'?'active':''}" onclick="changeStatus(${e.id},'new')">🆕 Новая</button>
          <button class="status-btn ${e.status==='confirmed'?'active':''}" onclick="changeStatus(${e.id},'confirmed')">✅ Подтв.</button>
          <button class="status-btn ${e.status==='done'?'active':''}" onclick="changeStatus(${e.id},'done')">🏁 Выполн.</button>
          <button class="status-btn ${e.status==='cancelled'?'active':''}" onclick="changeStatus(${e.id},'cancelled')">❌ Отмена</button>
        </div>
...
// Проверка пересечения записей с учётом длительности
function checkConflict(date, time, duration, excludeId = null) {
  ...
  return state.entries.find(e => {
    ...
    // Пересечение: начало одной внутри другой или наоборот
    return (start < eEnd && end > eStart);
  });
}

// Найти свободные слоты на день
...
function setupModal() {
  ...
  // Изменение времени/даты → обновить свободные слоты
...
function updateTimeEnd() {
  ...
  if (time && duration) {
    ...
    if (conflict) {
      conflictDiv.innerHTML = `⚠️ <b>Конфликт!</b> Пересекается с записью: <b>${conflict.name}</b> (${conflict.time} - ${calcEndTime(conflict.time, conflict.duration)})`;
...
function updateFreeSlots() {
  ...
  // Показываем только ближайшие 10 слотов
...
function openModal(entry = null) {
  ...
  // Активировать кнопку длительности
...
echo "  ⏱️  Длительность процедуры (30/60/90/120 мин)"
echo "  🎯 Автоматический расчёт времени окончания"
echo "  🟢 Показ свободных слотов на день"
echo "  ⚠️  Предупреждение о конфликтах"
echo "  🏷️ Статусы записей (Новая/Подтв./Выполн./Отмена)"
echo "  📊 Средний чек в статистике"
