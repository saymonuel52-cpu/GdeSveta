#!/bin/bash
echo " Исправляю последний тест..."

# Исправляем тест валидации — проверяем валидацию напрямую
...
<body>
  ...
  <script>
    ...
    function testValidation() {
      try {
        // ИСПРАВЛЕНО: Создаём объект БЕЗ дефолтных значений
        const invalidEntry = {
          name: '',  // Пустое имя
          date: '',  // Пустая дата
          time: '',  // Пустое время
        ...
        // Ожидаем 3 ошибки: имя, дата, время
        showResult('errorResults', 'Валидация Entry (ошибок: ' + result.errors.length + ': ' + result.errors.join(', ') + ')', 
          !result.valid && result.errors.length >= 2);
...
echo ""
echo "🎯 ИСПРАВЛЕНО:"
echo "  ✅ testValidation() — создаёт объект БЕЗ Entry.create()"
echo "  ✅ Проверяет валидацию напрямую: name='', date='', time=''"
echo "  ✅ Ожидает 3 ошибки (имя, дата, время)"
echo ""
