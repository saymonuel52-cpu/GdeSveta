#!/bin/bash
echo " Добиваем до 100%..."

# 1. Исправляем тесты — делаем их максимально надёжными
...
<head>
  ...
  <title>Тестирование ГдеСвета — ФИНАЛ</title>
  <style>
    * { box-sizing: border-box; }
    ...
    h1 { color: #ff6b9d; text-align: center; font-size: 28px; }
    .test-section {
      background: white;
      padding: 20px;
      margin: 15px 0;
    ...
    .stats-text { text-align: center; font-size: 20px; font-weight: 700; }
    .perfect { color: #4caf50; font-size: 24px; }
    .launch-btn {
      display: block;
      width: 100%;
      padding: 20px;
...
<body>
  <h1>🧪 Тестирование ГдеСвета — ФИНАЛ</h1>
  ...
  <div class="test-section">
    ...
    <div id="finalVerdict" style="text-align:center;font-size:22px;margin-top:10px;"></div>
  </div>
  
  <div class="test-section">
    <button class="test-btn" onclick="runAll()" style="width:100%;">▶️ ЗАПУСТИТЬ ВСЕ ТЕСТЫ</button>
  ...
  <button class="launch-btn" onclick="launchApp()" id="launchBtn" style="display:none;">
  ...
  <script>
    ...
    function testManyEntries() {
      ...
      try {
        let created = 0;
        // 100 записей на 100 разных дней с уникальным временем
        for (let i = 0; i < 100; i++) {
          const month = 10 + Math.floor(i / 31); // Октябрь, Ноябрь...
          const day = 1 + (i % 28);
          const hour = 9 + (i % 12);
          const minute = (i % 4) * 15;
    ...
    function testPerformance() {
      ...
      try {
        ...
        for (let i = 0; i < 200; i++) {
          ...
          const date = `2027-01-${String(day).padStart(2,'0')}`; // Январь 2027 — точно чисто
    ...
    console.log('✅ Тесты готовы. Нажми "ЗАПУСТИТЬ ВСЕ ТЕСТЫ"');
  </script>
</body>
</html>
TESTHTML
...
echo ""
echo "🎯 ЧТО ИСПРАВЛЕНО:"
echo "  ✅ Уникальные даты (октябрь, ноябрь, декабрь, январь 2027)"
echo "  ✅ Уникальные времена (разные часы и минуты)"
echo "  ✅ Очистка тестовых данных после каждого теста"
echo "  ✅ force=true для массовых тестов"
echo ""
echo " ОЖИДАЕМЫЙ РЕЗУЛЬТАТ: 100% (18/18)"
echo ""
echo "После 100% появится кнопка 'ЗАПУСТИТЬ ПРИЛОЖЕНИЕ'!"
