...
if [ "$files_ok" = false ]; then
  echo ""
  echo "❌ Не все файлы на месте! Исправь структуру."
  exit 1
fi

# 2. Создаём финальную тестовую страницу
...
<head>
  ...
  <title> Финальное тестирование ГдеСвета</title>
  <style>
    * { box-sizing: border-box; }
    ...
    h1 { color: #ff6b9d; text-align: center; font-size: 26px; }
    .test-section {
      background: white;
      padding: 20px;
      margin: 15px 0;
    ...
    .perfect { color: #4caf50; font-size: 28px; }
    .launch-btn {
      display: none;
      width: 100%;
      padding: 20px;
    ...
    .section-title { font-size: 18px; color: #ff6b9d; margin-top: 20px; margin-bottom: 10px; }
  </style>
</head>
<body>
  <h1>🧪 Финальное тестирование ГдеСвета</h1>
  ...
  <script>
    ...
    function updateStats() {
      ...
      if (percent === 100) {
        ...
        verdict.innerHTML = '<span style="color:#2196f3;">👍 Хорошо! ' + percent + '%</span>';
      } else {
        verdict.innerHTML = '<span style="color:#f44336;">⚠️ ' + percent + '% — есть проблемы</span>';
      }
    }
    ...
    function clearResults() {
      ['basicResults', 'modelResults', 'serviceResults', 'newFeatureResults', 'massResults', 'errorResults'].forEach(id => {
    ...
    function testUtils() {
      try {
        ...
        showResult('basicResults', `Utils: даты (${today}), конец (${end})`, 
    ...
    function testFamilyModel() {
      try {
        ...
        showResult('modelResults', 'FamilyMember: создание + иконка (' + icon + ')', m.id && icon === '');
    ...
    function testNoteService() {
      try {
        ...
        showResult('serviceResults', 'NoteService: CRUD', found);
    ...
    // === НОВЫЕ ФУНКЦИИ (5 приоритетов) ===
    function testNotificationService() {
      try {
        const status = NotificationService.getStatus();
        const hasPermission = status.permission === 'granted' || status.permission === 'default';
        showResult('newFeatureResults', 'NotificationService: инициализация (' + status.permission + ')', hasPermission && status.supported);
      } catch (e) { showResult('newFeatureResults', 'NotificationService: ' + e.message, false); }
    }
    
    function testFamilyShare() {
      try {
        // Тест PIN
        FamilyShare.setPin('1234');
        const checkOk = FamilyShare.checkPin('1234');
        const checkFail = !FamilyShare.checkPin('0000');
        FamilyShare.removePin();
        
        // Тест экспорта
        const today = Utils.getToday();
        const schedule = FamilyShare.exportDaySchedule(today);
        const hasExport = schedule.includes('ГдеСвета');
        
        showResult('newFeatureResults', 'FamilyShare: PIN (' + (checkOk && checkFail ? 'OK' : 'FAIL') + ') + экспорт', checkOk && checkFail && hasExport);
      } catch (e) { showResult('newFeatureResults', 'FamilyShare: ' + e.message, false); }
    }
    
    function testTasks() {
      try {
        const initialCount = FamilyShare.getTasks().length;
        const task = FamilyShare.addTask({ text: 'Тест задача', category: 'shopping' });
        const added = FamilyShare.getTasks().length === initialCount + 1;
        
        FamilyShare.toggleTask(task.id);
        const toggled = FamilyShare.getCompletedTasks().find(t => t.id === task.id);
        
        FamilyShare.deleteTask(task.id);
        const deleted = !FamilyShare.getTasks().find(t => t.id === task.id);
        
        showResult('newFeatureResults', 'Задачи: создание + переключение + удаление', added && toggled && deleted);
      } catch (e) { showResult('newFeatureResults', 'Tasks: ' + e.message, false); }
    }
    ...
    function testTemplates() {
      try {
        ...
        const hasDefaults = templates.length >= 4;
        
        const newTemplate = TemplateService.save({
          name: 'Тест шаблон',
          category: 'work',
          serviceName: 'Шугаринг',
          duration: 60,
          price: 1500
        });
        
        const applied = TemplateService.apply(newTemplate.id);
        const applyOk = applied && applied.name === 'Тест шаблон';
        
        TemplateService.delete(newTemplate.id);
        const deleted = !TemplateService.getAll().find(t => t.name === 'Тест шаблон');
        
        showResult('newFeatureResults', 'Шаблоны: инициализация + создание + применение', hasDefaults && applyOk && deleted);
      } catch (e) { showResult('newFeatureResults', 'Templates: ' + e.message, false); }
    }
    
    function testDarkTheme() {
      try {
        // Проверяем, что функция initTheme существует
        const hasFunction = typeof initTheme === 'function';
        // Проверяем, что CSS содержит dark-theme
        const styles = document.styleSheets;
        let hasDarkTheme = false;
        for (let sheet of styles) {
          try {
            for (let rule of sheet.cssRules) {
              if (rule.selectorText && rule.selectorText.includes('dark-theme')) {
                hasDarkTheme = true;
                break;
              }
            }
        ...
        showResult('newFeatureResults', 'Тёмная тема: функция + CSS', hasFunction && hasDarkTheme);
      } catch (e) { showResult('newFeatureResults', 'Dark theme: ' + e.message, false); }
    }
    ...
    function testValidation() {
      try {
        const invalidEntry = { name: '', date: '', time: '', category: 'work' };
        const result = Entry.validate(invalidEntry);
    ...
    function runAll() {
      ...
      // Новые функции (5 приоритетов)
      testNotificationService(); testFamilyShare(); testTasks(); testTemplates(); testDarkTheme();
      // Массовые
    ...
    console.log('✅ Финальные тесты готовы!');
  </script>
</body>
</html>
TESTHTML

echo "✅ Финальная тестовая страница создана"
...
echo ""
echo "════════════════════════════════════════"
echo "🧪 ЧТО БУДЕТ ТЕСТИРОВАТЬСЯ:"
echo "════════════════════════════════════════"
echo ""
echo "🔧 Базовые (4):"
echo "  • Storage, Events, Utils, Store"
echo ""
echo " Модели (4):"
echo "  • Entry, Note, PriceItem, FamilyMember"
echo ""
echo "⚙️ Сервисы (4):"
echo "  • EntryService, NoteService, PriceService, ConflictChecker"
echo ""
echo "🆕 Новые функции (5):"
echo "  • 🔔 NotificationService"
echo "  • 👨‍‍👧 FamilyShare (PIN + экспорт)"
echo "  • ✅ Задачи"
echo "  • ⚡ Шаблоны"
echo "  • 🌙 Тёмная тема"
echo ""
echo " Массовые (3):"
echo "  • 100 записей, 50 заметок, 200 записей + статистика"
echo ""
echo "️ Ошибки (3):"
echo "  • Валидация, конфликты, граничные случаи"
echo ""
echo "ИТОГО: 23 теста"
echo ""
echo "🎯 ОЖИДАЕМЫЙ РЕЗУЛЬТАТ: 95%+ (22/23)"
echo "   (NotificationService может не пройти без разрешения)"
